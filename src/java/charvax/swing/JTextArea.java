...
public class JTextArea
    ...
    public synchronized void append(String text_) {
        ...
        try{
          doc.insertString(doc.getLength(), text_, null);
        }catch(BadLocationException e){
          // this seems bad, but jdk does it???
        }
    ...
    public synchronized void insert(String text_, int pos_) {
        try{
          getDocument().insertString(pos_, text_, null);
        }catch(BadLocationException e){
          throw new IllegalArgumentException(e.getMessage());
        }
        setCaretPosition(pos_ + text_.length());
    ...
    public synchronized void replaceRange(String text_, int start_, int end_) {
        try{
          ...
          if(doc instanceof AbstractDocument){
              ((AbstractDocument)doc).replace(start_, end_ - start_, text_, null);
          }else{
            doc.remove(start_, end_ - start_);
            doc.insertString(start_, text_, null);
          }
        }catch(BadLocationException e){
          throw new IllegalArgumentException(e.getMessage());
        }
        setCaretPosition(start_ + text_.length());
    ...
    public void processKeyEvent(KeyEvent ke_) {
        ...
        } else if (key == KeyEvent.VK_BACK_SPACE || key == 127) {
            if(caret > 0){
    ...
    private int offsetCalc(int mode_, int value_) {
        ...
        if (mode_ == LINE_OF_OFFSET) {
            if (value_ == getDocument().getLength()) {
                return row;
            } else
                return lineOfOffset;
            ...
            throw new IndexOutOfBoundsException("Invalid offset or line number: mode=" + mode_ +
                    " value=" + value_ + " row=" + row + " doc=\"" + getDocument() + "\"");
        }
    }
